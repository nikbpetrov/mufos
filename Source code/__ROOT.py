...
raw_corr_table_style_Frame = ttk.LabelFrame(test_specifications_master_Frame, text="Correlations table styling")
...
summ_corr_table_style_Frame = ttk.LabelFrame(test_specifications_master_Frame, text="Correlations table styling")
...
spss_corr_table_style_Frame = ttk.LabelFrame(test_specifications_master_Frame, text="Correlations table styling")
...
ttk.Label(output_filename_Frame, text="Save FOrmatted table in:").grid(row=1, column=0, sticky="NW", padx=(0, 10))
...
def input_validation():
	...
	if global_vars.input_type == "raw":
		...
		if global_vars.output_filetype == "Word":
			raise Exception("You can only export to Word if the output is FOrmatted table. For a list of p values, please export to Excel.")
...
submit_button = ttk.Button(master, text="Submit", command=submit, width=10)
submit_button.grid(row=3, column=2, rowspan=2, sticky="ES", padx=(0, 15), pady=(5, 5))

def version_check():
	try:
		response = requests.get("https://api.github.com/repos/nikbpetrov/mufos/releases/latest", timeout=3)
		response_json = response.json()

		version_name = response_json["name"]
		version_message = response_json["body"]
		version_tag = response_json["tag_name"]
	except:
		return

	if version_tag != global_vars.version:

		top = tk.Toplevel()
		top.grab_set()
		top.title("New MUFOS version detected!")
		def exit(event):
			top.destroy()
		top.bind("<Escape>", exit)

		user_message = "A new version of MUFOS has been released: {v}\n\nThe changes associated with this version are: \n\n{c}\n\nYou're strongly encouraged to visit the software's repository or webpage to download the latest version.".format(v = version_name, c = version_message)

		tk.Label(top, text=user_message).grid(row=0, column=0, columnspan=2, sticky="NWES", padx=15, pady=(5, 0))
		ttk.Button(top, text="Software page", command=lambda url=global_vars.software_page: open_new(url)).grid(row=1, column=0, padx=15, pady=5)
		ttk.Button(top, text="Github repository", command=lambda url=global_vars.github_repository: open_new(url)).grid(row=1, column=1, padx=(0, 15), pady=5)
